...
import { CalcInput, DEFAULT_INPUT, calculate, LINING_TYPES } from "@/lib/calculator";
  ...
  const saveSpec = useMutation({
    mutationFn: async () => {
      ...
      const { error: rErr } = await supabase.from("lining_results").insert({
        ...
        gables_m2: result.gablesM2,
        total_m2: result.totalM2,
        walls_panels: result.wallsPanels,
        roof_panels: result.roofPanels,
        gable_panels: result.gablePanels,
